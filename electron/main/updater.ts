...
/**
 * Base CDN URL for the auto-update feed (without trailing channel path).
 * Set the KNA_UPDATE_FEED_URL env var at build/run time to enable auto-updates;
 * empty string disables the feed entirely (electron-updater stays no-op).
 * Until we stand up our own R2/OSS distribution, KNA Desktop ships without
 * auto-update — users will download fresh installers from wearekna.com.
 */
const OSS_BASE_URL = process.env.KNA_UPDATE_FEED_URL || '';
